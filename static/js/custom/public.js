function each_class (e){
    ...
    const scr = {md: "480px", lg: "767px", xl: "1023px",xxl: "1277px"};
    const isMd = (el ,name) => {
        const md = window.matchMedia(`(min-width: ${scr.md}) and (max-width: ${scr.lg})`)
        if(md.matches) set_css(name.slice(3), el)
        md.addEventListener("change", (e) => { if(e.matches) set_css(name.slice(3), el)
        })
    }
    const isLg = (el ,name) => {
        const lg = window.matchMedia(`(min-width: ${scr.lg}) and (max-width: ${scr.xl})`)
        if(lg.matches) set_css(name.slice(3), el)
        lg.addEventListener("change", (e) => {
            if(e.matches) set_css(name.slice(3), el)
        })
    }
    const isXl = (el ,name) => {
        const xl = window.matchMedia(`(min-width: ${scr.xl}) and (max-width: ${scr.xxl})`)
        if(xl.matches) set_css(name.slice(3), el)
        xl.addEventListener("change", (e) => {
            if(e.matches) set_css(name.slice(3), el)
        })
    }
    const isXxl = (el ,name) => {
        const xxl = window.matchMedia(`(min-width: ${scr.xxl})`)
        if(xxl.matches) set_css(name.slice(4), el)
        xxl.addEventListener("change", (e) => {
            if(e.matches) set_css(name.slice(4), el)
        })
    }
    ...
    const set_css = (classes, el) => {
        ...
        classes.split(" ").forEach( _ => {
            ...
            if ( _.startsWith("ml-") ) assignValue(el, "margin-left", _.split("-")[1]);
            if ( _.startsWith("mr-") ) assignValue(el, "margin-right", _.split("-")[1]);
            if ( _.startsWith("mlr-") ) assignValue(el, ["margin-left", "margin-right"], [_.split("-")[1], _.split("-")[1]]);
            if ( _.startsWith("mrl-") ) assignValue(el, ["margin-left", "margin-right"], [_.split("-")[1], _.split("-")[1]]);
            ...
            if ( _.startsWith("w-") ) assignValue(el, "width", _.split("-")[1]);
            ...
            if ( _.startsWith("line-") ) assignValue(el, "line-height", _.split("-")[1]);
            ...
            if ( _.startsWith("wght-") ) assignValue(el, "font-weight", _.split("-")[1]);
            if ( _.startsWith("backdropFilter-") ) assignValue(el, "backdrop-filter", px_rm(_.split("-")[1]));
            if ( _.startsWith("md:") ) isMd(el, _);
            if ( _.startsWith("lg:") ) isLg(el, _);
            if ( _.startsWith("xl:") ) isXl(el, _);
            if ( _.startsWith("xxl:") ) isXxl(el, _);
        });
